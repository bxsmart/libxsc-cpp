...
XscTcpServer::XscTcpServer(const string& ne , shared_ptr<XscLog> log) :
		XscServer(ne, log)
{
	this->proType = XscProtocolType::XSC_PROTOCOL_TCP;
}

bool XscTcpServer::startup(shared_ptr<XscCfg> c)
{
	shared_ptr<XscTcpCfg> cfg = static_pointer_cast<XscTcpCfg>(c);
	if (cfg->worker < 1)
	{
	...
	LOG_INFO("xsc tcp server startup successful, server-name: %s, addr: %s, worker: %d", this->ne.c_str(), this->cfg->addr.c_str(), this->cfg->worker)
	return true;
}

bool XscTcpServer::publish()
{
	string ip;
	int port;
	...
	for (uint i = 0; i < this->cfg->worker; ++i)
	{
		shared_ptr<XscTcpWorker> worker = static_pointer_cast<XscTcpWorker>(this->xscWorker.at(i));
